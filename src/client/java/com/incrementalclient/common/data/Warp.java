...
public enum Warp {
    ...
    public boolean isAtPosition(double tolerance) {
        if (MinecraftClient.getInstance().player != null){
            return isAtPosition(MinecraftClient.getInstance().player.getBlockPos(), TOLERANCE);
        }
        return false;
    }

    public boolean isAtPosition(BlockPos blockPos) {
        return isAtPosition(blockPos, TOLERANCE);
    }

    public boolean isAtPosition(BlockPos blockPos, double tolerance) {
        if (MinecraftClient.getInstance().world != null) {
            var currentWorld = World.findById(MinecraftClient.getInstance().world.getRegistryKey().getValue());
